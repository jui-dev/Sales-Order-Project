...
class PickingListObserver
{
    ...
    private function finaliseStockMovements(PickingList $list): void
    {
        DB::transaction(function () use ($list) {
            ...
            // Mark completed timestamp if absent. Stamped before posting, because
            // the entry is dated by it.
            if (empty($list->completed_at)) {
                $list->updateQuietly(['completed_at' => now()]);
            }

            // Goods have left on a sale, so the ledger stops carrying them as
            // inventory and starts carrying them as cost. What that entry looks
            // like is CostOfGoodsSoldPostingRule's business; whether it applies
            // at all - only sales, never transfers - is the rule's decision too.
            $this->ledger->postFor($list);
