            ...
            if ($request->status === 'confirmed' && $order->status !== 'confirmed') {
                ...
                try {
                    ...
                    return redirect()->route('invoices.show', $invoice->id)
                        ->with('success', 'Order marked as completed and invoice generated successfully.');
                } catch (\Throwable $e) {
                    return redirect()->route('invoices.index')
                        ->with('success', 'Order marked as completed. Invoice generation failed: ' . $e->getMessage());
                }
            } else {
                $order->update(['status' => $request->status]);
